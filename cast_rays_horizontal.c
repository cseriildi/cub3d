...
/*   Updated: 2024/12/06 17:44:48 by icseri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

static void	horizontal_no_clue(t_ray_trace_state *ray_state, double player_x,
...
static void	horizontal_still_no_clue(t_ray_trace_state *ray_state, t_data *data)
{
	//rename this
...
static void	horizontal_no_clue_again(t_ray_trace_state *ray_state,
		...
		&& ray_state.cur_y >= 0 && ray_state.cur_y < data->map.row)
	{
			...
			|| data->map.map[ray_state.map_y][ray_state.map_x] == 'D')
		{
			...
			if (ray_state.dist < ray->closest_distance)
				horizontal_no_clue_again(&ray_state, ray, data);
			break ;
		}
